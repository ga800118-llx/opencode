...
- 成功运行：[GitHub Actions 31377997106](https://github.com/ga800118-llx/opencode/actions/runs/31377997106)，attempt 1
- Actions artifacts：delivery `9059092850`；evidence `9059093191`
...
受版本控制证据目录：`docs/product/evidence/windows/0.1.0-alpha.2/`

本机完整交付与复核副本位于 Git 忽略的 `packages/desktop/dist/internal-beta/0.1.0-alpha.2/`。

受版本控制证据与本机 artifact 副本逐字节一致，未经改写。三份 JSON 仅包含经过严格字段白名单投影的字段，不包含原始日志或绝对路径；两份启动日志仅复制到凭据扫描输入中，不作为原始证据交付。
...
`SHA256SUMS.txt` 只列出前五个交付载荷，不列出清单自身。上述文件是 run `31377997106` attempt 1 的原始交付文件，从 artifact 下载并精确分段重组后未修改；本地仅用 `SHA256SUMS.txt` 复核五个载荷，并对便携 ZIP 和 388066331 字节的 GitHub artifact 外层 ZIP 执行完整 `unzip -t`，结果均通过。
...
- 安装器配置为 per-user、`perMachine: false`、`allowElevation: false`，不请求管理员权限；静默自定义目录安装退出码为 0。自动化在 hosted runner 上完成，标准非管理员 Windows 11 账户仍待实机验证。
...
- 安装器和便携包均自带运行所需的 Git；测试者不需要预装 Git、Bun 或 Node.js。
...
CI 结果来自 run `31377997106` attempt 1。下表标注“本地”的结果均在可执行文件源码提交 `d2880064ce62162f4bc22ee22f6919887350a901` 的代码状态上执行，属于未归档的本地观察。

| 检查 | 结果 |
| --- | --- |
| Desktop Windows 全量测试 | CI（run `31377997106` attempt 1）224 pass，0 fail；本地复跑 224 pass，0 fail |
| App Windows 全量单元测试 | CI（run `31377997106` attempt 1）1046 pass，4 skip，0 fail；共 1050 项、138 个文件 |
...
| Desktop、App、OpenCode 类型检查 | 本地全部通过 |
| pre-push 依赖包类型检查 | 本地 30/30 通过 |

## 结论与限制

该版本已准备交给少量可信 Windows 11 x64 测试者开展首次真实机器测试；测试者不需要预装 Git、Bun 或 Node.js。安装器配置为 per-user、`perMachine: false`、`allowElevation: false`，不请求管理员权限；当前自动化在 Windows Server 2025 hosted runner 上完成，标准非管理员 Windows 11 账户及其他 Windows 11 x64 实机验证仍待完成，不能表述为已完成 Windows 11 x64 验证。
