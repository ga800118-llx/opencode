...
test("keeps Home task search focus on the combobox during keyboard navigation", async ({ page }) => {
  ...
  const controlsID = await search.getAttribute("aria-controls")
  expect(controlsID).toBeTruthy()
  if (!controlsID) throw new Error("Home task search combobox must control a listbox")
  const listbox = page.locator(`[id=${JSON.stringify(controlsID)}]`)
