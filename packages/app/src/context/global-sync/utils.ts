...
export function normalizeAgentList(input: AgentListOutput["data"] | Agent[]): Agent[] {
  ...
  return (input as AgentListOutput["data"]).map((agent) => {
    const request = agent.request as
      | AgentListOutput["data"][number]["request"]
      | { readonly headers: Record<string, string>; readonly body: Record<string, unknown> }
    const options = "settings" in request ? request.settings : request.body
    return {
      name: agent.id,
