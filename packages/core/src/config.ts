...
export interface Interface {
  ...
  /** Reads location config entries from disk without changing the cached location configuration. */
  readonly reloadEntries?: () => Effect.Effect<Entry[]>
}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadEntries = Effect.fn("Config.loadEntries")(function* () {
    ...
    // Read configuration once when this location opens. General consumers keep
    // this snapshot until the location is reopened.
    const configs = yield* loadEntries()
