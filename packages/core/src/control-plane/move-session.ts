...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const moveSession = Effect.fn("MoveSession.moveSession")(function* (input: Input) {
      const recorded = yield* sessions.get(input.sessionID)
      if (!recorded) return yield* new SessionV2.NotFoundError({ sessionID: input.sessionID })
      const directory = AbsolutePath.make(input.destination.directory)
      if (recorded.location.directory === directory) return

      return yield* execution.handoff(
        input.sessionID,
        Effect.gen(function* () {
...
export const node = makeGlobalNode({
  service: Service,
  layer,
  deps: [Git.node, EventV2.node, ProjectV2.node, SessionExecution.node, SessionStore.node],
})
