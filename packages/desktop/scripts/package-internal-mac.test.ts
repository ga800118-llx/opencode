...
describe("internal Mac package", () => {
  ...
  test("uses an isolated deterministic build environment and rejects external bundled Git", () => {
    const environment = createInternalMacBuildEnvironment(
      {
        ...
        GUAI_CODE_BUILD_COMMIT: "f".repeat(40),
      },
      {
    ...
    expect(environment).toEqual({
      ...
      GUAI_CODE_BUILD_COMMIT: "a".repeat(40),
  ...
  test("requires no configured models and rejects implicit runtime dependencies in a clean packaged profile", async () => {
    ...
    await Bun.write(
      ...
      JSON.stringify({
        provider: { unexpected: {} },
        enabled_providers: ["unexpected"],
        disabled_providers: [],
        model: "unexpected/model",
      }),
    )
    await expect(verifyInternalMacRuntimeState(directory)).rejects.toThrow(
      "Clean packaged runtime exposed models before user configuration",
    )
