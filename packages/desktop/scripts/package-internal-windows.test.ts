...
describe("internal Windows package", () => {
  test("uses the Windows internal Beta package metadata", () => {
    expect(pkg.version).toBe("0.1.0-alpha.4")
    expect(pkg.description).toBeTruthy()
    expect(pkg.scripts["package:win:internal"]).toBe("bun ./scripts/package-internal-windows.ts")
  })

  test("identifies Alpha 4 artifacts and states the Windows testing and SmartScreen constraints", async () => {
    const guide = await Bun.file(guidePath).text()

    expect(guide).toContain("版本：`0.1.0-alpha.4`")
    expect(guide).toContain("Guai-Code-Beta-0.1.0-alpha.4-win-x64.exe")
    expect(guide).toContain("Guai-Code-Beta-0.1.0-alpha.4-win-x64-portable.zip")
