...
describe("internal Windows package", () => {
  ...
  test("identifies Alpha 5 artifacts and states the Windows testing and SmartScreen constraints", async () => {
    const guide = await Bun.file(guidePath).text()

    expect(guide).toContain("版本：`0.1.0-alpha.5`")
    expect(guide).toContain("Guai-Code-Beta-0.1.0-alpha.5-win-x64.exe")
    expect(guide).toContain("Guai-Code-Beta-0.1.0-alpha.5-win-x64-portable.zip")
