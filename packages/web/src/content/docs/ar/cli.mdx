---
title: CLI
description: أوامر وخيارات CLI في Guai Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Guai Code افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
opencode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Guai Code برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Guai Code.
...
يتضمن CLI الخاص بـ Guai Code أيضا الأوامر التالية.

---

### agent

إدارة وكلاء Guai Code.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Guai Code قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
opencode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Guai Code تعمل عن بعد. على سبيل المثال:
...
يعتمد Guai Code على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `opencode auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

عند تشغيل Guai Code يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Guai Code.
...
تشغيل Guai Code في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم Guai Code قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `opencode`) |
...
بدء خادم Guai Code بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
opencode serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف Guai Code دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Guai Code لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Guai Code.

```bash
opencode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Guai Code.
...
بدء خادم Guai Code بدون واجهة مع واجهة ويب.

```bash
opencode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Guai Code عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
إلغاء تثبيت Guai Code وإزالة جميع الملفات ذات الصلة.
...
تحديث Guai Code إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر Guai Code الخيارات العامة التالية.
...
يمكن تهيئة Guai Code باستخدام متغيرات البيئة.
