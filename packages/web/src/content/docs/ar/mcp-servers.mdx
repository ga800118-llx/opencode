...
يمكنك إضافة أدوات خارجية إلى Guai Code باستخدام _Model Context Protocol_، أو MCP. يدعم Guai Code خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Guai Code](https://opencode.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى Guai Code تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Guai Code بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Guai Code المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `opencode mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Guai Code الرموز بشكل آمن في `~/.local/share/opencode/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Guai Code إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Guai Code مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Guai Code بحساب Sentry الخاص بك.
