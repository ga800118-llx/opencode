---
title: المزوّدون
description: استخدام أي موفّر LLM في Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Guai Code ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Guai Code.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Guai Code.
...
اختبارها والتحقق من أنها تعمل بشكل جيد مع Guai Code. [اعرف المزيد](/docs/zen).
...
يعمل مثل أي مزوّد آخر في Guai Code واستخدامه اختياري بالكامل.

---

## OpenCode Go

OpenCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Guai Code.
...
لاستخدام Amazon Bedrock مع Guai Code:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل Guai Code:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Guai Code غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين Guai Code لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل Guai Code بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات Guai Code.
...
تخزين الرمز في مساحة تخزين مصادقة Guai Code.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Guai Code نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Guai Code
...
لاستخدام اشتراك GitHub Copilot مع Guai Code:
...
لاستخدام Google Vertex AI مع Guai Code:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر Guai Code، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Guai Code تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Guai Code مدرجة كجلسة مستقلة.
...
يمكنك ضبط Guai Code لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط Guai Code لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط Guai Code لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Guai Code. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Guai Code:
...
3. انسخ مفتاح API لاستخدامه في Guai Code.
...
6. **مهم**: قبل استخدام نماذج السحابة في Guai Code، يجب سحب معلومات النموذج محليا:
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات Guai Code.
...
5. يمكنك أيضا تخصيصها عبر إعدادات Guai Code. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات Guai Code. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع Guai Code. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
تسمح حقول `limit` لـ Guai Code بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
2. للمزوّدات المخصّصة، راجع إعدادات Guai Code وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات Guai Code.
