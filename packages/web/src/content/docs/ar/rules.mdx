---
title: القواعد
description: اضبط تعليمات مخصصة لـ Guai Code.
---

يمكنك تزويد Guai Code بتعليمات مخصصة عبر إنشاء ملف `AGENTS.md`. هذا مشابه لقواعد Cursor. يحتوي هذا الملف على تعليمات تُضاف إلى سياق نموذج اللغة (LLM) لتخصيص سلوكه لمشروعك المحدد.

---

## التهيئة

لإنشاء ملف `AGENTS.md` جديد، يمكنك تشغيل الأمر `/init` في Guai Code.

:::tip
يُستحسن عمل commit لملف `AGENTS.md` الخاص بمشروعك إلى Git.
:::

سيقوم ذلك بفحص مشروعك وكل محتوياته لفهم طبيعة المشروع وتوليد ملف `AGENTS.md` بناءً على ذلك. يساعد هذا Guai Code على التنقّل داخل المشروع بشكل أفضل.
...
يدعم Guai Code أيضًا قراءة ملف `AGENTS.md` من مواقع متعددة، ولكل موقع غرض مختلف.
...
يمكنك أيضًا وضع قواعد عامة في ملف `~/.config/opencode/AGENTS.md`. تُطبَّق هذه القواعد على جميع جلسات Guai Code.
...
للمستخدمين الذين ينتقلون من Claude Code، يدعم Guai Code اصطلاحات ملفات Claude Code كبدائل احتياطية:
...
عند بدء Guai Code، يبحث عن ملفات القواعد بهذا الترتيب:
...
على الرغم من أن Guai Code لا يقوم تلقائيًا بتحليل مراجع الملفات داخل `AGENTS.md`، يمكنك تحقيق وظيفة مشابهة بطريقتين:
...
يمكنك تعليم Guai Code قراءة ملفات خارجية عبر تقديم تعليمات صريحة داخل `AGENTS.md`. إليك مثالًا عمليًا:
...
- ضمان أن Guai Code يحمّل الملفات فقط عند الحاجة للمهمة المحددة
