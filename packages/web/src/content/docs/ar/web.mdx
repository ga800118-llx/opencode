---
title: الويب
description: استخدام Guai Code في متصفحك.
---

يمكن تشغيل Guai Code كتطبيق ويب داخل متصفحك، ليمنحك تجربة البرمجة القوية بالذكاء الاصطناعي نفسها دون الحاجة إلى terminal.

![Guai Code Web - جلسة جديدة](../../../assets/web/web-homepage-new-session.png)
...
يؤدي ذلك إلى تشغيل خادم محلي على `127.0.0.1` بمنفذ عشوائي متاح، ويفتح Guai Code تلقائيا في المتصفح الافتراضي لديك.
...
افتراضيا، يختار Guai Code منفذا متاحا. يمكنك تحديد منفذ:
...
افتراضيا، يرتبط الخادم بـ `127.0.0.1` (للاستخدام المحلي فقط). لجعل Guai Code متاحا على شبكتك:

```bash
opencode web --hostname 0.0.0.0
```

عند استخدام `0.0.0.0` سيعرض Guai Code كلا من عناوين الوصول المحلي وعناوين الشبكة:
...
بعد التشغيل، تتيح لك واجهة الويب الوصول إلى جلسات Guai Code الخاصة بك.

### الجلسات

اعرض جلساتك وأدرها من الصفحة الرئيسية. يمكنك رؤية الجلسات النشطة وبدء جلسات جديدة.

![Guai Code Web - جلسة نشطة](../../../assets/web/web-homepage-active-session.png)

### حالة الخادم

انقر على "See Servers" لعرض الخوادم المتصلة وحالتها.

![Guai Code Web - See Servers (عرض الخوادم)](../../../assets/web/web-homepage-see-servers.png)
