...
يعمل Zen مثل أي مزوّد آخر في Guai Code. تسجّل الدخول إلى OpenCode Zen وتحصل على
مفتاح API. استخدامه **اختياري بالكامل** ولا تحتاج إليه لاستخدام Guai Code.
...
:::tip
اختبرنا مجموعة منتقاة من النماذج والمزوّدين الذين يعملون جيدا مع Guai Code.
:::
...
يعمل OpenCode Zen مثل أي مزوّد آخر في Guai Code.
...
يستخدم [معرّف النموذج](/docs/config/#models) في إعدادات Guai Code الصيغة `opencode/<model-id>`. على سبيل المثال، بالنسبة إلى GPT 5.5، ستستخدم `opencode/gpt-5.5` في إعداداتك.
...
قد تلاحظ [نماذج منخفضة التكلفة](/docs/config/#models)، مثل Haiku أو Nano أو Flash، في سجل الاستخدام. يستخدم Guai Code هذه النماذج لإنشاء عناوين الجلسات.
...
4. **عدم فرض أي ارتباط حصري** عبر تمكينك من استخدامه مع أي عامل برمجة آخر. ومع إبقاء إمكانية استخدام أي مزوّد آخر مع Guai Code أيضا دائما.
