---
title: Konfiguracija
description: Korištenje Guai Code JSON konfiguracije.
---

Možete konfigurirati Guai Code koristeći JSON konfiguracijski fajl.

---

## Format

Guai Code podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Postavite svoju globalnu Guai Code konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Kada se Guai Code pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Guai Code server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Guai Code konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Guai Code pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Guai Code će se vratiti na `"build"` s upozorenjem.
...
Guai Code će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Guai Code **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Guai Code sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite Guai Code da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
