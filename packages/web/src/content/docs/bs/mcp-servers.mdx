...
Možete dodati vanjske alate u Guai Code koristeći _Model Context Protocol_, ili MCP. Guai Code podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Guai Code Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Guai Code automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Guai Code će:
...
Ako server zahtijeva autentifikaciju, Guai Code će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Guai Code sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Guai Code, zajedno s ugrađenim alatima. Možete njima upravljati kroz Guai Code konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Guai Code sa vašim Sentry nalogom.
