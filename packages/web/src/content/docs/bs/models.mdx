...
Guai Code koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM providera**, uključujući lokalne modele.

## Provajderi

Većina popularnih provajdera su unaprijed učitani prema zadanim postavkama. Ako ste dodali vjerodajnice za provajdera putem naredbe `/connect`, oni će biti dostupni kada pokrenete Guai Code.
...
Evo nekoliko modela koji dobro rade sa Guai Code, bez posebnog redosleda. (Ovo nije potpuna lista niti je nužno ažurirana):
...
Mnogi modeli podržavaju više varijanti sa različitim konfiguracijama. Guai Code se isporučuje sa ugrađenim podrazumevanim varijantama za popularne provajdere.

### Ugrađene varijante

Guai Code se isporučuje sa zadanim varijantama za mnoge provajdere:
**Anthropic**:
...
Kada se Guai Code pokrene, on provjerava modele u sljedećem prioritetnom redoslijedu:

1. Oznaka komandne linije `--model` ili `-m`. Format je isti kao u konfiguracijskoj datoteci: `provider_id/model_id`.
2. Lista modela u Guai Code konfiguraciji.
