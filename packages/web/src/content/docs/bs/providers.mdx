---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Guai Code koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Guai Code konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Guai Code
config.
...
testirano i potvrđeno da dobro radi sa Guai Code. [Saznajte više](/docs/zen).
...
Radi kao i svaki drugi provajder u Guai Code i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s Guai Code:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete Guai Code:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Guai Code.
:::
...
Možete konfigurirati Guai Code za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi Guai Code ispravno radio.
   :::
...
Također možete dodati modele kroz svoju Guai Code konfiguraciju.
...
da pohrani token u Guai Code auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Guai Code koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Guai Code
...
Da biste koristili svoju GitHub Copilot pretplatu s Guai Code:
...
Za korištenje Google Vertex AI s Guai Code:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće Guai Code.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Guai Code razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Guai Code razgovor naveden kao zasebna sesija.
...
Možete konfigurirati Guai Code za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati Guai Code za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati Guai Code za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Guai Code. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Guai Code:
...
3. Kopirajte API ključ za korištenje u Guai Code.
...
6. **Važno**: Prije upotrebe modela oblaka u Guai Code, morate lokalno povući informacije o modelu:
...
Također možete dodati dodatne modele putem vaše Guai Code konfiguracije.
...
5. Također ih možete prilagoditi putem vaše Guai Code konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju Guai Code konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
Polja `limit` omogućavaju Guai Code da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite Guai Code konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj Guai Code konfiguraciji.
