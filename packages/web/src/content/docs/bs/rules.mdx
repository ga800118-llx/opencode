---
title: Pravila
description: Postavite prilagodena uputstva za Guai Code.
---

Mozete dodati prilagodena uputstva za Guai Code tako sto kreirate `AGENTS.md` datoteku. Ovo je slicno pravilima u Cursor-u. Sadrzi uputstva koja se ubacuju u LLM kontekst da prilagode ponasanje za vas projekat.

---

## Inicijalizacija

Da kreirate novu `AGENTS.md` datoteku, pokrenite `/init` komandu u Guai Code.

:::tip
Preporuceno je da `AGENTS.md` iz projekta commitujete u Git.
:::

Ovo skenira projekat i njegov sadrzaj, razumije cemu projekat sluzi i generise `AGENTS.md`. Tako Guai Code bolje navigira kroz kod.
...
Guai Code podrzava citanje `AGENTS.md` datoteke sa vise lokacija. Svaka lokacija ima drugu svrhu.
...
Mozete imati i globalna pravila u `~/.config/opencode/AGENTS.md`. Ona se primjenjuju u svim Guai Code sesijama.
...
Za korisnike koji prelaze sa Claude Code, Guai Code podrzava i Claude konvencije datoteka kao rezervu:
...
Kada se Guai Code pokrene, trazi datoteke pravila ovim redoslijedom:
...
Iako Guai Code ne parsira automatski reference datoteka u `AGENTS.md`, slicno ponasanje mozete dobiti na dva nacina:
...
Mozete nauciti Guai Code da cita eksterne datoteke tako sto cete dati eksplicitna uputstva u `AGENTS.md`. Evo prakticnog primjera:
...
- Osigurate da Guai Code ucitava datoteke samo kad su potrebne za konkretan zadatak
