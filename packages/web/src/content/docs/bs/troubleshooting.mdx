...
Da biste otklonili probleme s Guai Code, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Guai Code pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
Guai Code Desktop pokreće lokalni Guai Code server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Guai Code` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
Guai Code također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Guai Code mogao ponovo izgraditi.

1. Potpuno zatvorite Guai Code Desktop.
...
3. Ponovo pokrenite Guai Code Desktop.

---

### Rješavanje problema sa vezom na serveru

Guai Code Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, Guai Code Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Guai Code karakteristikama.

---

### Obavještenja se ne prikazuju

Guai Code Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Guai Code u postavkama vašeg OS-a, i
...
1. Zatvorite Guai Code Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Guai Code Desktop aplikacije):
...
Ako imate problema s Guai Code:
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Guai Code dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite Guai Code da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Guai Code da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Guai Code će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
