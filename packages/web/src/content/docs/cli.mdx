---
title: CLI
description: Guai Code CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Guai Code CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with Guai Code programmatically.
...
Start the Guai Code terminal user interface.
...
The Guai Code CLI also has the following commands.

---

### agent

Manage agents for Guai Code.
...
Attach a terminal to an already running Guai Code backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote Guai Code backend. For example:
...
Guai Code is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When Guai Code starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Guai Code.
...
Run Guai Code in non-interactive mode by passing a prompt directly.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach to a running Guai Code server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `opencode`) |
...
Start a headless Guai Code server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
opencode serve
```

This starts an HTTP server that provides API access to Guai Code functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Show token usage and cost statistics for your Guai Code sessions.
...
Import session data from a JSON file or Guai Code share URL.

```bash
opencode import <file>
```

You can import from a local file or an Guai Code share URL.
...
Start a headless Guai Code server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access Guai Code through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run Guai Code.
...
Uninstall Guai Code and remove all related files.
...
Updates Guai Code to the latest version or a specific version.
...
The Guai Code CLI takes the following global flags.
...
Guai Code can be configured using environment variables.
