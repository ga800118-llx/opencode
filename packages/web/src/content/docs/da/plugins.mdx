---
title: Plugins
description: Skriv dine egne plugins for at udvide Guai Code.
---

Plugins giver dig mulighed for at udvide Guai Code ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Guai Codes standardadfærd.
...
Guai Code kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En Guai Code SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide Guai Code.
...
:::note
Hvis du bruger Guai Code desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå Guai Code i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til Guai Code:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som Guai Code kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for Guai Code sammen med indbyggede værktøjer.
