...
For at fejlfinde problemer med Guai Code, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Guai Code gemmer sessionsdata og andre applikationsdata på disken på:
...
Guai Code Desktop kører en lokal opencode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Guai Code`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
Guai Code kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Guai Code kan genopbygge den.

1. Afslut Guai Code Desktop helt.
2. Slet cache-mappen:
...
3. Start Guai Code Desktop på ny.

---

### Løs problemer med serverforbindelse

Guai Code Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
På Windows kræver Guai Code Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Guai Code.

---

### Meddelelser vises ikke

Guai Code Desktop viser kun systemvarsler når:

- varsler er aktiveret for Guai Code i dine OS-indstillinger, og
...
1. Afslut Guai Code Desktop.
2. Find og slet disse filer (de findes i Guai Code Desktop-appens datamappe):
...
Hvis du oplever problemer med Guai Code:
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. Guai Code installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start Guai Code på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge Guai Code til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
Guai Code vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
