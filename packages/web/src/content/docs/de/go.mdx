...
Go funktioniert wie jeder andere Provider in Guai Code. Du abonnierst OpenCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Guai Code zu verwenden.
...
OpenCode Go funktioniert wie jeder andere Provider in Guai Code.
...
Die [Modell-ID](/docs/config/#models) in deiner Guai Code Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `opencode-go/kimi-k3` in deiner Config verwenden.
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Guai Code zu nutzen.
