---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Guai Code verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Guai Code-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Guai Code anpassen
config.
...
Getestet und verifiziert, dass es gut mit Guai Code funktioniert. [Learn more](/docs/zen).
...
Es funktioniert wie jeder andere Anbieter in Guai Code und ist völlig optional.

---

## OpenCode Go

OpenCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom OpenCode-Team getestet und verifiziert wurden, dass sie gut mit Guai Code funktionieren.
...
So verwenden Sie Amazon Bedrock mit Guai Code:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Guai Code ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Guai Code wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können Guai Code so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Guai Code ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Guai Code-Konfiguration hinzufügen.
...
um das Token im Guai Code-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Guai Code verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Guai Code
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Guai Code:
...
So verwenden Sie Google Vertex AI mit Guai Code:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Guai Code ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Guai Code konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede Guai Code-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Guai Code-Konversation als separate Sitzung aufgeführt.
...
Sie können Guai Code für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Guai Code über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Guai Code über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Guai Code konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit Guai Code:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Guai Code.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Guai Code verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Guai Code-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Guai Code-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Guai Code:
...
Sie können Modelle auch über Ihre Guai Code-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Guai Code verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann Guai Code erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Guai Code-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Guai Code-Konfiguration übereinstimmt.
