---
title: Regeln
description: Lege eigene Anweisungen fuer Guai Code fest.
---

Du kannst Guai Code ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in Guai Code den Befehl `/init` aus.
...
So kann Guai Code sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht Guai Code sie zu erweitern.
...
Guai Code kann `AGENTS.md` aus mehreren Orten lesen.
...
Diese gelten in allen Guai Code-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Guai Code dessen Dateikonventionen als Fallback:
...
Beim Start sucht Guai Code in dieser Reihenfolge nach Regeldateien:
...
Guai Code parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
...
Du kannst Guai Code explizit anweisen, externe Dateien zu lesen.
