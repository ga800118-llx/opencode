...
Wenn Guai Code Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Guai Code speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Guai Code Desktop startet im Hintergrund einen lokalen Guai Code-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Guai Code`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Guai Code kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Guai Code ihn neu aufbauen kann.

1. Beende Guai Code Desktop komplett.
...
3. Starte Guai Code Desktop neu.

---

### Server-Verbindungsprobleme beheben

Guai Code Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt Guai Code Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Guai Code-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Guai Code Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Guai Code in den OS-Einstellungen aktiviert sind, und
...
1. Beende Guai Code Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Guai Code Desktop):
...
Wenn du Probleme mit Guai Code hast:
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. Guai Code installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte Guai Code neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt Guai Code, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
Guai Code erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
