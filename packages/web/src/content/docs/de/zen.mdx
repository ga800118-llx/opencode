...
Zen funktioniert wie jeder andere Provider in Guai Code. Du meldest dich bei OpenCode Zen an und erhältst deinen API-Key. Es ist **vollständig optional** und du musst es nicht verwenden, um Guai Code zu nutzen.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Providern getestet, die gut mit Guai Code funktionieren.
:::
...
OpenCode Zen funktioniert wie jeder andere Provider in Guai Code.
...
Die [Model-ID](/docs/config/#models) in deiner Guai Code-Konfiguration verwendet das Format `opencode/<model-id>`. Für GPT 5.5 würdest du zum Beispiel `opencode/gpt-5.5` in deiner Konfiguration verwenden.
...
Möglicherweise siehst du [kostengünstige Modelle](/docs/config/#models) wie Haiku, Nano oder Flash in deinem Nutzungsverlauf. Guai Code verwendet diese Modelle, um Session-Titel zu generieren.
...
4. **keinen Lock-in** zu haben, indem du es mit jedem anderen Coding-Agenten verwenden kannst. Und dir außerdem immer zu erlauben, mit Guai Code auch jeden anderen Provider zu verwenden.
