---
title: CLI
description: Opciones y comandos de la CLI de Guai Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Guai Code de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Guai Code mediante programación.
...
Inicie la interfaz de usuario del terminal Guai Code.
...
La CLI de Guai Code también tiene los siguientes comandos.

---

### agent

Administrar agentes para Guai Code.
...
Conecte una terminal a un servidor backend Guai Code que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto Guai Code. Por ejemplo:
...
Guai Code funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opencode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Cuando se inicia Guai Code, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Guai Code.
...
Ejecute Guai Code en modo no interactivo pasando un mensaje directamente.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor Guai Code en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `opencode`) |
...
Inicie un servidor Guai Code sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
opencode serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad Guai Code sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Administrar Guai Code sesiones.
...
Enumere todas las sesiones Guai Code.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Guai Code.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Guai Code.

```bash
opencode import <file>
```

Puede importar desde un archivo local o una URL compartida Guai Code.
...
Inicie un servidor Guai Code sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Guai Code a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale Guai Code y elimine todos los archivos relacionados.
...
Actualiza Guai Code a la última versión o a una versión específica.
...
La CLI de Guai Code toma las siguientes banderas globales.
...
Guai Code se puede configurar mediante variables de entorno.
