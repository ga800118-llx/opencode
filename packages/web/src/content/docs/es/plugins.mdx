---
title: Complementos
description: Escriba sus propios complementos para extender Guai Code.
---

Los complementos le permiten extender Guai Code al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Guai Code.
...
Guai Code ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK Guai Code para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar Guai Code.
...
:::note
Si está utilizando la aplicación de escritorio Guai Code, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que Guai Code lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a Guai Code:
...
El ayudante `tool` crea una herramienta personalizada a la que Guai Code puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para Guai Code junto con las herramientas integradas.
