---
title: Proveedores
description: Usando cualquier proveedor LLM en Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Guai Code usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Guai Code.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Guai Code
configuración.
...
probado y verificado para funcionar bien con Guai Code. [Más información](/docs/zen).
...
Funciona como cualquier otro proveedor en Guai Code y su uso es completamente opcional.
...
probado y verificado para funcionar bien con Guai Code.
...
Para usar Amazon Bedrock con Guai Code:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta Guai Code:
...
:::info
El uso de su suscripción Claude Pro/Max en Guai Code no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar Guai Code para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que Guai Code funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración Guai Code.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Guai Code`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación Guai Code.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Guai Code utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Guai Code
...
Para utilizar su suscripción GitHub Copilot con Guai Code:
...
Para utilizar Google Vertex AI con Guai Code:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta Guai Code.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de Guai Code, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Guai Code como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Guai Code enumerada como una sesión separada.
...
Puede configurar Guai Code para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar Guai Code para usar modelos locales a través de LM Studio.
...
Puedes configurar Guai Code para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Guai Code. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Guai Code:
...
3. Copie la clave API para usarla en Guai Code.
...
6. **Importante**: Antes de usar modelos en la nube en Guai Code, debe obtener la información del modelo localmente:
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración Guai Code.
...
5. También puedes personalizarlos a través de tu configuración Guai Code. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Guai Code:
...
También puede personalizar modelos a través de su configuración Guai Code. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con Guai Code. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
Los campos `limit` le permiten a Guai Code comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
2. Para proveedores personalizados, verifique la configuración Guai Code y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración Guai Code.
