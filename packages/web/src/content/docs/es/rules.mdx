---
title: Reglas
description: Establezca instrucciones personalizadas para Guai Code.
---

Puede proporcionar instrucciones personalizadas a Guai Code creando un archivo `AGENTS.md`. Esto es similar a las reglas del Cursor. Contiene instrucciones que se incluirán en el contexto de LLM para personalizar su comportamiento para su proyecto específico.

---

## Inicializar

Para crear un nuevo archivo `AGENTS.md`, puede ejecutar el comando `/init` en Guai Code.

:::tip
Debes enviar el archivo `AGENTS.md` de tu proyecto a Git.
:::

Esto escaneará su proyecto y todo su contenido para comprender de qué se trata y generar un archivo `AGENTS.md` con él. Esto ayuda a Guai Code a navegar mejor por el proyecto.
...
Guai Code también admite la lectura del archivo `AGENTS.md` desde múltiples ubicaciones. Y esto tiene diferentes propósitos.
...
También puedes tener reglas globales en un archivo `~/.config/opencode/AGENTS.md`. Esto se aplica en todas las sesiones Guai Code.
...
Para los usuarios que migran desde Claude Code, Guai Code admite las convenciones de archivos de Claude Code como alternativas:
...
Cuando se inicia Guai Code, busca archivos de reglas en este orden:
...
Si bien Guai Code no analiza automáticamente las referencias de archivos en `AGENTS.md`, puedes lograr una funcionalidad similar de dos maneras:
...
Puede enseñarle a Guai Code a leer archivos externos proporcionando instrucciones explícitas en su `AGENTS.md`. He aquí un ejemplo práctico:
...
- Asegúrese de que Guai Code cargue archivos solo cuando sea necesario para la tarea específica
