...
Para depurar problemas con Guai Code, comience verificando los registros y los datos locales que almacena en el disco.
...
Guai Code almacena datos de sesión y otros datos de aplicaciones en el disco en:
...
Guai Code Desktop ejecuta un servidor Guai Code local (el sidecar `opencode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Guai Code` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
Guai Code también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Guai Code pueda reconstruirlo.

1. Salga de Guai Code Desktop por completo.
...
3. Reinicie el escritorio Guai Code.

---

### Solucionar problemas de conexión del servidor

Guai Code Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
En Windows, el escritorio Guai Code requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Guai Code.

---

### Notificaciones que no se muestran

Guai Code Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Guai Code en la configuración de su sistema operativo, y
...
1. Salga del escritorio Guai Code.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Guai Code):
...
Si tiene problemas con Guai Code:
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. Guai Code instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
2. Reinicie Guai Code para reinstalar los paquetes del proveedor más recientes.

Esto obligará a Guai Code a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
Guai Code detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
