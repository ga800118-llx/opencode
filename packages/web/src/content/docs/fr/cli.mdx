---
title: CLI
description: Options et commandes de la CLI Guai Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI Guai Code démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
opencode
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec Guai Code par programme.
...
Démarrez l’interface utilisateur du terminal Guai Code.
...
La CLI Guai Code dispose également des commandes suivantes.

---

### agent

Gérer les agents pour Guai Code.
...
Attachez un terminal à un serveur backend Guai Code déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
opencode attach [url]
```

Cela permet d'utiliser le TUI avec un backend Guai Code distant. Par exemple :
...
Guai Code est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `opencode auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Lorsque Guai Code démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans Guai Code.
...
Exécutez Guai Code en mode non interactif en transmettant directement un prompt.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Connectez-vous à un serveur Guai Code en cours d'exécution (par exemple, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Mot de passe d'authentification de base (par défaut `OPENCODE_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OPENCODE_SERVER_USERNAME` ou `opencode`) |
...
Démarrez un serveur Guai Code sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
opencode serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'Guai Code sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Liste toutes les sessions Guai Code.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions Guai Code.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage Guai Code.

```bash
opencode import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage Guai Code.
...
Démarrez un serveur Guai Code sans interface graphique avec une interface Web.

```bash
opencode web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à Guai Code via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Désinstallez Guai Code et supprimez tous les fichiers associés.
...
Met à jour Guai Code vers la dernière version ou une version spécifique.
...
La CLI Guai Code prend les flags globaux suivants.
...
Guai Code peut être configuré à l'aide de variables d'environnement.
