...
Vous pouvez ajouter des commandes personnalisées via la configuration Guai Code ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre Guai Code [config](/docs/config) :
...
Guai Code comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
