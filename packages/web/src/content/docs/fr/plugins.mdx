---
title: Plugins
description: Écrivez vos propres plugins pour étendre Guai Code.
---

Les plugins vous permettent d'étendre Guai Code en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Guai Code.
...
Guai Code exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK Guai Code pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre Guai Code.
...
:::note
Si vous utilisez l'application de bureau Guai Code, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez Guai Code de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à Guai Code :
...
L'assistant `tool` crée un outil personnalisé que Guai Code peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour Guai Code aux côtés des outils intégrés.
