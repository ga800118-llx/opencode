---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Guai Code utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Guai Code.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Guai Code
configuration.
...
testé et vérifié pour fonctionner correctement avec Guai Code. [En savoir plus](/docs/zen).
...
Il fonctionne comme n’importe quel autre fournisseur dans Guai Code et son utilisation est totalement facultative.
...
testé et vérifié pour fonctionner correctement avec Guai Code.
...
Pour utiliser Amazon Bedrock avec Guai Code :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de Guai Code :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Guai Code n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer Guai Code pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que Guai Code fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration Guai Code.
...
Guai Code utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Guai Code
...
Pour utiliser Google Vertex AI avec Guai Code :
...
```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id Guai Code
```
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Guai Code en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Guai Code répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour Guai Code. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Guai Code :
...
3. Copiez la clé API à utiliser dans Guai Code.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Guai Code, vous devez extraire les informations du modèle localement :
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' Guai Code
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group Guai Code
   ```
...
Les champs `limit` permettent à Guai Code de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
