...
Pour déboguer les problèmes avec Guai Code, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Guai Code stocke les données de session et autres données d'application sur le disque à l'emplacement :
...
Guai Code Desktop exécute un serveur Guai Code local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Guai Code` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
Guai Code peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Guai Code puisse le reconstruire.

1. Quittez complètement Guai Code Desktop.
...
3. Redémarrez le bureau Guai Code.

---

### Résoudre les problèmes de connexion au serveur

Guai Code Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Sur Windows, Guai Code Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Guai Code.

---

### Les notifications ne s'affichent pas

Guai Code Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Guai Code dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Guai Code.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Guai Code Desktop) :
...
Si vous rencontrez des problèmes avec Guai Code :
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. Guai Code installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
2. Redémarrez Guai Code pour réinstaller les derniers packages du fournisseur

Cela forcera Guai Code à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
Guai Code détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
