---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Guai Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Guai Code fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Guai Code démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Guai Code TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Guai Code. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez Guai Code. *Alias* : `/quit`, `/q`
...
- `leader_timeout` - Contrôle la durée pendant laquelle Guai Code attend après la leader key. Valeur par défaut : `2000`.
