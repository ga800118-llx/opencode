...
Zen fonctionne comme n'importe quel autre fournisseur dans Guai Code. Vous vous connectez à OpenCode Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Guai Code.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec Guai Code.
:::
...
OpenCode Zen fonctionne comme n'importe quel autre fournisseur dans Guai Code.
...
Le [model id](/docs/config/#models) dans votre configuration Guai Code utilise le format `opencode/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `opencode/gpt-5.5` dans votre configuration.
...
Vous remarquerez peut-être des [modèles à faible coût](/docs/config/#models), tels que Haiku, Nano ou Flash, dans votre historique d'utilisation. Guai Code utilise ces modèles pour générer les titres des sessions.
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec Guai Code également.
