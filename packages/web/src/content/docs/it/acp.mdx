---
title: Supporto ACP
description: Usa Guai Code in qualunque editor compatibile con ACP.
---

Guai Code supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Guai Code via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia Guai Code come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Guai Code' nel selettore agenti di AI Chat.
...
Per usare Guai Code come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Guai Code come agente ACP per la chat.
...
Guai Code funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Guai Code
