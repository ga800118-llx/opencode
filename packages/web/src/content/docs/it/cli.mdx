---
title: CLI
description: Opzioni e comandi della CLI di Guai Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Guai Code, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Guai Code in modo programmatico.
...
Avvia l'interfaccia testuale di Guai Code (interfaccia utente da terminale).
...
La CLI di Guai Code include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Guai Code.
...
Collega un terminale a un backend server di Guai Code già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend Guai Code remoto. Per esempio:
...
Guai Code si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando Guai Code si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Guai Code.
...
Esegui Guai Code in modalità non interattiva passando un prompt direttamente.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server Guai Code in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `opencode`) |
...
Avvia un server Guai Code headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
opencode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di Guai Code senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Gestisci le sessioni Guai Code.
...
Elenca tutte le sessioni Guai Code.
...
Mostra statistiche di utilizzo token e costo per le sessioni Guai Code.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Guai Code.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione Guai Code.
...
Avvia un server Guai Code headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a Guai Code tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla Guai Code e rimuove tutti i file correlati.
...
Aggiorna Guai Code all'ultima versione o a una versione specifica.
...
La CLI di Guai Code accetta i seguenti flag globali.
...
Guai Code può essere configurato tramite variabili d'ambiente.
