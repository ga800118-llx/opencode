...
Go funziona come qualsiasi altro provider in Guai Code. Ti abboni a OpenCode Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare Guai Code.
...
OpenCode Go funziona come qualsiasi altro provider in Guai Code.
...
Il [model id](/docs/config/#models) nella tua Guai Code config
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Guai Code.
