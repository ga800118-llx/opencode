---
title: Plugin
description: Scrivi plugin per estendere Guai Code.
---

I plugin ti permettono di estendere Guai Code agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Guai Code.
...
Guai Code esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di Guai Code per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere Guai Code.
...
:::note
Se usi l'app desktop di Guai Code, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a Guai Code di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a Guai Code:
...
L'helper `tool` crea uno strumento personalizzato che Guai Code può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in Guai Code insieme agli strumenti integrati.
