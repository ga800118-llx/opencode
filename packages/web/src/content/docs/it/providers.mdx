---
title: Provider
description: Usa qualsiasi provider LLM in Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Guai Code usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Guai Code.
...
verificati per funzionare bene con Guai Code. [Scopri di piu](/docs/zen).
...
Funziona come qualsiasi altro provider in Guai Code ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Guai Code:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui Guai Code:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Guai Code non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare Guai Code per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché Guai Code funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di Guai Code.
...
salvare il token nello storage di auth di Guai Code.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Guai Code usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Guai Code
...
Per usare il tuo abbonamento GitHub Copilot con Guai Code:
...
Per usare Google Vertex AI con Guai Code:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui Guai Code.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da Guai Code, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Guai Code come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Guai Code elencata come sessione separata.
...
Puoi configurare Guai Code per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare Guai Code per usare modelli locali tramite LM Studio.
...
Puoi configurare Guai Code per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Guai Code. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Guai Code:
...
3. Copia la chiave API da usare in Guai Code.
...
6. **Importante**: Prima di usare modelli cloud in Guai Code, devi scaricare le informazioni del modello localmente:
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di Guai Code.
...
5. Puoi anche personalizzarli tramite la configurazione di Guai Code. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di Guai Code. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con Guai Code. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
I campi `limit` permettono a Guai Code di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
2. Per provider personalizzati, controlla la config di Guai Code e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config Guai Code.
