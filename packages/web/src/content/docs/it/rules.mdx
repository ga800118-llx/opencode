---
title: Regole
description: Imposta istruzioni personalizzate per Guai Code.
---

Puoi fornire istruzioni personalizzate a Guai Code creando un file `AGENTS.md`. E' simile alle regole di Cursor. Contiene istruzioni che verranno incluse nel contesto dell'LLM per personalizzarne il comportamento per il tuo progetto.

---

## Inizializza

Per creare un nuovo file `AGENTS.md`, puoi eseguire il comando `/init` in Guai Code.

:::tip
Dovresti committare il file `AGENTS.md` del progetto in Git.
:::

Questo scansiona il progetto e tutti i suoi contenuti per capire di cosa si tratta e generare un file `AGENTS.md`. Questo aiuta Guai Code a orientarsi meglio nel progetto.
...
Guai Code supporta anche la lettura del file `AGENTS.md` da piu' posizioni, e questo serve a scopi diversi.
...
Puoi anche avere regole globali in un file `~/.config/opencode/AGENTS.md`. Queste si applicano a tutte le sessioni di Guai Code.
...
Per gli utenti che migrano da Claude Code, Guai Code supporta come fallback le convenzioni di file di Claude Code:
...
Quando Guai Code si avvia, cerca i file di regole in questo ordine:
...
Anche se Guai Code non interpreta automaticamente i riferimenti a file in `AGENTS.md`, puoi ottenere una funzionalita' simile in due modi:
...
Puoi insegnare a Guai Code a leggere file esterni fornendo istruzioni esplicite nel tuo `AGENTS.md`. Ecco un esempio pratico:
...
- Assicurati che Guai Code carichi file solo quando servono per il task specifico
