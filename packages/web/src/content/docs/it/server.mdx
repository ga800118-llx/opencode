---
title: Server
description: Interagisci con il server Guai Code via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `opencode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client Guai Code.
...
:::tip
Usa il server Guai Code per interagire con Guai Code in modo programmatico.
:::

Questa architettura permette a Guai Code di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `opencode serve` per avviare un server standalone. Se la TUI di Guai Code e gia in esecuzione, `opencode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Guai Code.
...
Il server Guai Code espone le seguenti API.
