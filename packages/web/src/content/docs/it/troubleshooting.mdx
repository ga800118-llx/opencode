...
Per diagnosticare problemi con Guai Code, inizia controllando i log e i dati locali che salva su disco.
...
Guai Code salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
Guai Code Desktop esegue in background un server locale di Guai Code (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Guai Code` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
Guai Code puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Guai Code possa ricostruirla.

1. Chiudi completamente Guai Code Desktop.
...
3. Riavvia Guai Code Desktop.

---

### Risolvi problemi di connessione al server

Guai Code Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, Guai Code Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Guai Code.

---

### Notifiche non visualizzate

Guai Code Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Guai Code nelle impostazioni del sistema operativo, e
...
1. Chiudi Guai Code Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Guai Code Desktop):
...
Se riscontri problemi con Guai Code:
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. Guai Code installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
2. Riavvia Guai Code per reinstallare i pacchetti provider piu recenti

Questo forzera Guai Code a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
Guai Code rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
