---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Guai Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Guai Code fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Guai Code avvia la TUI per la directory corrente.
...
Quando usi la TUI di Guai Code, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Guai Code. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Guai Code. _Alias_: `/quit`, `/q`
...
- `leader_timeout` - Controlla per quanto tempo Guai Code attende dopo il leader key. Default: `2000`.
