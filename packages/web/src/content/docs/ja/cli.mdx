---
title: CLI
description: Guai Code CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Guai Code CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
opencode
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、Guai Code をプログラム的に操作できるようになります。
...
Guai Code ターミナルユーザーインターフェイスを開始します。
...
Guai Code CLI には次のコマンドもあります。

---

### agent

Guai Code のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の Guai Code バックエンドサーバーにターミナルを接続します。

```bash
opencode attach [url]
```

これにより、リモート Guai Code バックエンドで TUI を使用できるようになります。例えば：
...
Guai Code は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`opencode auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/opencode/auth.json` に保存されます。

```bash
opencode auth login
```

Guai Code が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを Guai Code で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで Guai Code を実行します。
...
| <nobr><code>{"--attach"}</code></nobr>   |          | 実行中の Guai Code サーバー (http://localhost:4096 など) に接続します。                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`     | Basic 認証パスワード（デフォルトは `OPENCODE_SERVER_PASSWORD`）                           |
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `opencode`）         |
...
API アクセスのためにヘッドレス Guai Code サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
opencode serve
```

これにより、TUI インターフェイスを使用せずに Guai Code 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opencode`)。
...
Guai Code セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは Guai Code 共有 URL からセッションデータをインポートします。

```bash
opencode import <file>
```

ローカルファイルまたは Guai Code 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス Guai Code サーバーを起動します。

```bash
opencode web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて Guai Code にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opencode`)。
...
Guai Code をアンインストールし、関連ファイルをすべて削除します。
...
Guai Code を最新バージョンまたは特定のバージョンに更新します。
...
Guai Code CLI は次のグローバルフラグを受け取ります。
