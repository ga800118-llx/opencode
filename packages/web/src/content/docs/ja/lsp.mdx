---
title: LSPサーバー
description: Guai Code は LSP サーバーと統合します。
---

Guai Code は Language Server Protocol (LSP) サーバーと統合し、診断を agent へのフィードバックとして使用できます。

---

## 組み込み

Guai Code には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
LSP が有効で Guai Code がファイルを開くと、次のようになります。
...
Guai Code 設定の `lsp` セクションを通じて LSP サーバーを有効化し、カスタマイズできます。
