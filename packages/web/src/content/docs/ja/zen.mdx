...
Zen は Guai Code のほかのプロバイダーと同じように動作します。OpenCode Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Guai Code を使うために必須ではありません。
...
:::tip
Guai Code でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
OpenCode Zen は、Guai Code のほかのプロバイダーと同じように動作します。
...
Guai Code 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `opencode/gpt-5.5` を使用します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。Guai Code はこれらのモデルをセッションタイトルの生成に使用します。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Guai Code では常にほかのプロバイダーも使えるようにすること。
