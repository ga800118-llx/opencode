---
title: 규칙
description: Guai Code에 대한 사용자 정의 지침 설정.
---

`AGENTS.md` 파일을 만들 수 있도록 Guai Code에 사용자 정의 지침을 제공 할 수 있습니다. Cursor의 규칙과 유사합니다. LLM의 컨텍스트에 포함된 지침을 통해 특정 프로젝트에 대한 행동을 사용자 정의합니다.

---

## 초기화

새로운 `AGENTS.md` 파일을 만들려면 `/init` 명령을 Guai Code에서 실행할 수 있습니다.

:::tip
프로젝트의 `AGENTS.md` 파일을 Git에 투입해야 합니다.
:::

프로젝트가 무엇인지 이해하고 `AGENTS.md` 파일을 생성합니다. 이 도움말 Guai Code는 프로젝트를 더 잘 탐색합니다.
...
Guai Code는 또한 다수 위치에서 `AGENTS.md` 파일을 읽습니다. 그리고 이것은 다른 목적을 봉사합니다.
...
또한 `~/.config/opencode/AGENTS.md` 파일에서 글로벌 규칙을 가질 수 있습니다. 이 모든 Guai Code 세션에서 적용됩니다.

이것은 Git에 전념하지 않거나 팀과 공유하지 않기 때문에 LLM이 따르는 모든 개인 규칙을 지정하는 것이 좋습니다.

### Claude 코드 호환성

Claude Code에서 마이그레이션하는 경우, Guai Code는 Claude Code의 파일 컨벤션을 fallbacks로 지원합니다.
...
Guai Code가 시작될 때, 이 순서에 있는 규칙 파일을 찾습니다:
...
Guai Code가 `AGENTS.md`에 있는 자동적으로 파스 파일 참조를 하지 않는 동안, 당신은 두 가지 방법으로 유사한 기능을 달성할 수 있습니다:
...
- 특정 작업에 필요한 경우에만 Guai Code 로드 파일 유지
