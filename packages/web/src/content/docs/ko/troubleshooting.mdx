...
Guai Code와 문제를 디버그하려면 로그와 로컬 데이터를 확인하여 디스크에 저장합니다.
...
Guai Code 저장 세션 데이터 및 디스크에 다른 응용 데이터:
...
Guai Code 데스크톱은 배경에서 로컬 Guai Code 서버 (`opencode-cli` sidecar)를 실행합니다. 대부분의 문제는 misbehaving 플러그인, 손상된 캐시, 또는 나쁜 서버 설정에 의해 발생합니다.

## 빠른 확인

- 완전히 종료하고 앱을 다시 시작.
- 앱이 오류 화면을 보여 주면 ** Restart**를 클릭하고 오류 세부 정보를 복사합니다.
- macOS만: `Guai Code` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
Guai Code는 디스크에서 로컬 플러그인을로드 할 수 있습니다. Temporarily 이동 이러한 방법 (또는 폴더 이름을 변경) 및 데스크톱 응용 프로그램을 다시 시작:
...
플러그인을 비활성화하는 경우 도움이되지 않습니다 (또는 플러그인 설치가 붙어있다), 캐시를 삭제 그래서 Guai Code는 그것을 재구성 할 수 있습니다.

1. Quit Guai Code 데스크톱 완전히.
2. 캐시 디렉토리 삭제:
...
3. Restart Guai Code 데스크탑.

---

## 서버 연결 문제 수정

Guai Code 데스크톱은 자체 로컬 서버(기본)를 시작하거나 구성된 서버 URL에 연결할 수 있습니다.
...
Windows에서 Guai Code 데스크톱은 Microsoft Edge ** WebView2 실행 시간 **를 요구합니다. 앱이 공백 창에 열거나 시작하지 않을 경우, install/update WebView2를 설치하고 다시 시도하십시오.

---

## Windows: 일반 성능 문제

느린 성능, 파일 액세스 문제 또는 Windows의 terminal 문제를 경험하는 경우 [WSL (Windows Subsystem for Linux)](/docs/windows-wsl)를 사용하여 시도하십시오. WSL은 Guai Code의 기능으로 더 원활하게 작동하는 Linux 환경을 제공합니다.

---

## 알림이 표시되지 않음

Guai Code 데스크톱은 시스템 알림을 보여줍니다 :

- OS 설정에서 Guai Code에 대한 알림이 활성화되고,
- 앱 창이 집중되지 않습니다.
...
1. Quit Guai Code 데스크탑.
2. 이 파일을 찾아 삭제 (Guai Code 데스크톱 앱 데이터 디렉토리에서 라이브):
...
API 호출 오류가 발생하면, 이 공급 업체 패키지로 인해 발생할 수 있습니다. Guai Code 동적 설치 공급자 패키지 (OpenAI, Anthropic, Google 등) 필요 하 고 로컬로 캐시.
...
2. 최신 공급자 포장을 재설치하는 Restart Guai Code

이것은 종종 모델 매개 변수와 API 변경과 호환성 문제를 해결하는 공급자 패키지의 가장 최근 버전을 다운로드하기 위해 Guai Code를 강제합니다.
...
Guai Code는 당신이 Wayland를 사용하고 `wl-clipboard`를 선호하는 경우에 검출할 것입니다, 그렇지 않으면의 순서에 있는 클립보드 공구를 찾아낼 것입니다: `xclip`와 `xsel`.
