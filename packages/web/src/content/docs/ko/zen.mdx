...
Zen은 Guai Code의 다른 provider와 똑같이 작동합니다. OpenCode Zen에 로그인해 API 키를 받으면 됩니다. 이는 **완전히 선택 사항**이며, Guai Code를 사용하기 위해 반드시 사용할 필요는 없습니다.
...
:::tip
Guai Code에서 잘 작동하는 일부 모델과 provider를 직접 테스트했습니다.
:::
...
OpenCode Zen은 Guai Code의 다른 provider와 똑같이 작동합니다.
...
Guai Code config에서 사용하는 [모델 ID](/docs/config/#models)는 `opencode/<model-id>` 형식입니다. 예를 들어 GPT 5.5를 사용하려면 config에서 `opencode/gpt-5.5`를 사용하면 됩니다.
...
사용 기록에서 Haiku, Nano 또는 Flash와 같은 [저비용 모델](/docs/config/#models)을 볼 수 있습니다. Guai Code는 이러한 모델을 사용해 세션 제목을 생성합니다.
...
4. 다른 코딩 에이전트와도 함께 사용할 수 있게 해 **lock-in이 없도록** 하기 위해. 또한 Guai Code에서 언제나 다른 provider도 사용할 수 있게 하기 위해.
