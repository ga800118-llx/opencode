...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Guai Code.
...
Here are several models that work well with Guai Code, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Many models support multiple variants with different configurations. Guai Code ships with built-in default variants for popular providers.

### Built-in variants

Guai Code ships with default variants for many providers:
...
When Guai Code starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Guai Code config.
