---
title: ACP-støtte
description: Bruk Guai Code i en hvilken som helst ACP-kompatibel editor.
---

Guai Code støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke Guai Code via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `opencode acp`.

Kommandoen starter Guai Code som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'Guai Code'-agenten i AI Chat-agentvelgeren.
...
For å bruke Guai Code som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke Guai Code som ACP-agent for chat.
...
Guai Code fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
