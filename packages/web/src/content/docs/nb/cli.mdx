---
title: CLI
description: Guai Code CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Guai Code CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
opencode
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Guai Code programmatisk.
...
Start Guai Code-terminalbrukergrensesnittet.
...
Guai Code CLI har også følgende kommandoer.

---

### agent

Administrer agenter for Guai Code.
...
Koble en terminal til en allerede kjørende Guai Code-backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Guai Code-backend. For eksempel:
...
Guai Code drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `opencode auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når Guai Code starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Guai Code.
...
Kjør Guai Code i ikke-interaktiv modus ved å sende en melding direkte.
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende Guai Code-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `opencode`) |
...
Start en headless Guai Code-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
opencode serve
```

Dette starter en HTTP-server som gir API tilgang til Guai Code-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Vis tokenbruk og kostnadsstatistikk for Guai Code-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Guai Code share-URL.

```bash
opencode import <file>
```

Du kan importere fra en lokal fil eller en Guai Code share-URL.
...
Start en headless Guai Code-server med et webgrensesnitt.

```bash
opencode web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Guai Code via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Avinstaller Guai Code og fjern alle relaterte filer.
...
Oppdaterer Guai Code til siste versjon eller en spesifikk versjon.
...
Guai Code CLI bruker følgende globale flagg.
...
Guai Code kan konfigureres ved hjelp av miljøvariabler.
