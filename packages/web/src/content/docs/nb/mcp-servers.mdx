...
Du kan legge til eksterne verktøy i Guai Code ved å bruke _Model Context Protocol_ eller MCP. Guai Code støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Guai Code Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Guai Code håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Guai Code:
...
Hvis serveren krever autentisering, vil Guai Code be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Guai Code lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Guai Code, sammen med innebygde verktøy. Så du kan administrere dem gjennom Guai Code-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Guai Code til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Guai Code-samtale oppført som en separat økt.
