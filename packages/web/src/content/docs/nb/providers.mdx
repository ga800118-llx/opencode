---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Guai Code bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Guai Code-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Guai Code
konfig.
...
testet og verifisert for å fungere godt med Guai Code. [Finn ut mer](/docs/zen).
...
Det fungerer som alle andre leverandører i Guai Code og er helt valgfritt å bruke.
...
testet og verifisert for å fungere godt med Guai Code.
...
Slik bruker du Amazon Bedrock med Guai Code:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Guai Code:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Guai Code skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Guai Code-konfigurasjonen.
...
for å lagre token i Guai Code auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Guai Code bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Guai Code
...
Slik bruker du GitHub Copilot-abonnementet med Guai Code:
...
Slik bruker du Google Vertex AI med Guai Code:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Guai Code.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Guai Code, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Guai Code-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Guai Code-samtale oppført som en separat økt.
...
Du kan konfigurere Guai Code for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Guai Code for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Guai Code for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Guai Code. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Guai Code:
...
3. Kopier API-nøkkelen for bruk i Guai Code.
...
6. **Viktig**: Før du bruker skymodeller i Guai Code, må du hente modellinformasjonen lokalt:
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Guai Code-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Guai Code-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Guai Code:
...
Du kan også tilpasse modeller gjennom Guai Code-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Guai Code. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar Guai Code forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk Guai Code-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Guai Code-konfigurasjonen.
