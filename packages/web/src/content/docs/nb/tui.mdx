...
Guai Code gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Guai Code starter TUI for gjeldende katalog.
...
Når du bruker Guai Code TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Guai Code. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Guai Code. _Aliaser_: `/quit`, `/q`
...
- `leader_timeout` - Styrer hvor lenge Guai Code venter etter leader key. Standard er `2000`.
