...
Możesz dodać dodatkowe informacje do Guai Code za pomocą _Model Context Protocol_, w skrócie MCP. Guai Code obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [Guai Code Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Guai Code automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, Guai Code:
...
Jeśli serwer wymaga uwierzytelnienia, Guai Code poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `opencode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji Guai Code bezpieczne przechowa tokeny w `~/.local/share/opencode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w Guai Code, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie Guai Code, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć Guai Code z kontem Sentry.
