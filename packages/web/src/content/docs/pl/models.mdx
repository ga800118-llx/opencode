...
Guai Code używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługi uruchamiania modeli pierwszych.

---

## Dostawcy

Dostępna usługa dostawcy jest dostępna. Jeśli otrzymasz poświadczenie dostawcy za pomocą polecenia `/connect`, będzie on dostępny po uruchomieniu Guai Code.
...
Oto kilka modeli, które dobrze współpracują z Guai Code, w kolejności. (Nie jest to lista wyczerpująca i nie jest konieczna aktualna):
...
Wiele modeli obsługuje wiele wariantów lub różnych wybranych. Guai Code jest dostarczany z podstawowymi wariantami wariantów dla dostawców.

### Wbudowane warianty

Guai Code jest dostarczany z domyślnymi wariantami dla wielu dostawców:
...
Po uruchomieniu Guai Code sprawdzanie modeli w następującej kolejności:

1. Flaga wiersza autora `--model` lub `-m`. Format jest taki sam jak w pliku konfiguracyjnym: `provider_id/model_id`.

2. Lista modeli w konstrukcji Guai Code.
