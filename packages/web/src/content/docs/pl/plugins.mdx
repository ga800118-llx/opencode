---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć Guai Code.
---

Wtyczki umożliwiające wyłączenie Guai Code poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania Guai Code.
...
Guai Code uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z Guai Code do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do Guai Code.
...
:::note
Jeśli korzystasz z aplikacji komputerowej Guai Code, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj Guai Code czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do Guai Code:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod Guai Code. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
