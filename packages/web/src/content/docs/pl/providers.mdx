---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Guai Code używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji Guai Code.
...
Możesz dostosować dostawców za pomocą sekcji `provider` w swoim Guai Code
config.
...
przetestowane i zweryfikowane, aby dobrze współpracować z Guai Code. [Dowiedz się więcej](/docs/zen).
...
Działa jak każdy inny dostawca w Guai Code i jest całkowicie opcjonalny w użyciu.

---

## OpenCode Go

OpenCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół OpenCode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z Guai Code.
...
Aby używać Amazon Bedrock z Guai Code:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania Guai Code:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w Guai Code nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować Guai Code do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod Guai Code działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji Guai Code.
...
Guai Code używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować Guai Code
...
Aby korzystać z subskrypcji GitHub Copilot z kodem Guai Code:
...
Aby używać Google Vertex AI z Guai Code:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania Guai Code.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą Guai Code, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację Guai Code jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja Guai Code będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować Guai Code tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować Guai Code tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować Guai Code tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem Guai Code. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z Guai Code:
...
3. Skopiuj klucz API do użycia w Guai Code.
...
6. **Ważne**: Przed użyciem modeli chmurowych w Guai Code musisz pobrać informacje o modelu lokalnie:
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji Guai Code.
...
5. Możesz je także dostosować za pomocą konfiguracji Guai Code. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z Guai Code:
...
Możesz także dostosować modele za pomocą konfiguracji Guai Code. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z Guai Code. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
Pola `limit` pozwalają Guai Code zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację Guai Code i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji Guai Code.
