---
title: Zasady
description: Ustaw niestandardowe instrukcje dla Guai Code.
---

Możesz dostarczyć niestandardowe instrukcje do Guai Code, tworząc plik `AGENTS.md`. Jest to podobne do zasad Cursora. Zawiera instrukcje, które zostaną uwzględnione w kontekście LLM, aby dostosować jego zachowanie do konkretnego projektu.
...
Guai Code obsługuje także odczytywanie pliku `AGENTS.md` z wielu lokalizacji. A to służy różnym celom.
...
Możesz także mieć reguły globalne w pliku `~/.config/opencode/AGENTS.md`. Jest to stosowane we wszystkich sesjach Guai Code.
...
W przypadku użytkowników migrujących z Claude Code Guai Code obsługuje konwencje plików Claude Code jako rozwiązania awaryjne:
...
Po uruchomieniu Guai Code szuka plików reguł w następującej kolejności:
...
Chociaż Guai Code nie analizuje automatycznie odniesień do plików w `AGENTS.md`, możesz osiągnąć podobną funkcjonalność na dwa sposoby:
...
Możesz nauczyć Guai Code czytać pliki zewnętrzne, podając wyraźne instrukcje w swoim `AGENTS.md`. Oto praktyczny przykład:
...
- Upewnij się, że Guai Code ładuje pliki tylko wtedy, gdy są potrzebne do konkretnego zadania
