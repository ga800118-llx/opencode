...
Aby debugować problemy z Guai Code, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
Guai Code przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
Guai Code Desktop uruchamia w tle lokalny serwer Guai Code (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Guai Code` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
Guai Code może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby Guai Code mógł ją odbudować.

1. Całkowicie zamknij Guai Code Desktop.
...
3. Uruchom ponownie Guai Code Desktop.

---

### Napraw problemy z połączeniem z serwerem

Guai Code Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
W systemie Windows Guai Code Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami Guai Code.

---

### Brak powiadomień

Guai Code Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla Guai Code w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie Guai Code Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji Guai Code Desktop):
...
Jeśli masz problemy z Guai Code:
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. Guai Code dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod Guai Code, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to Guai Code do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
Guai Code wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
