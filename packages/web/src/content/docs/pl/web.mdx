---
title: Web
description: Korzystanie z Guai Code w przeglądarce.
---

Guai Code może działać jako aplikacja internetowa w przeglądarce, zapewniając takie same możliwości kodowania AI bez konieczności korzystania z terminala.

![Sieć Guai Code - Nowa sesja](../../../assets/web/web-homepage-new-session.png)
...
Spowoduje to uruchomienie lokalnego serwera na `127.0.0.1` z losowo dostępnym portem i automatyczne otwarcie Guai Code w domyślnej przeglądarce.
...
Domyślnie Guai Code wybiera dostępny port. Możesz określić port:
...
Domyślnie serwer łączy się z `127.0.0.1` (tylko localhost). Aby udostępnić Guai Code w swojej sieci:

```bash
opencode web --hostname 0.0.0.0
```

Podczas korzystania z `0.0.0.0` Guai Code wyświetli zarówno adresy lokalne, jak i sieciowe:
...
Po uruchomieniu interfejs sieciowy zapewnia dostęp do sesji Guai Code.

### Sesje

Przeglądaj sesje i zarządzaj nimi ze strony głównej. Możesz zobaczyć aktywne sesje i rozpocząć nowe.

![Guai Code Web - Aktywna sesja](../../../assets/web/web-homepage-active-session.png)
