---
title: Providers
description: Using any LLM provider in Guai Code.
---

import config from "../../../config.mjs"
export const console = config.console

Guai Code uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Guai Code config.
...
You can customize the providers through the `provider` section in your Guai Code
config.
...
tested and verified to work well with Guai Code. [Learn more](/docs/zen).
...
It works like any other provider in Guai Code and is completely optional to use.
...
tested and verified to work well with Guai Code.
...
To use Amazon Bedrock with Guai Code:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running Guai Code:
...
Guai Code. Anthropic explicitly prohibits this.

Previous versions of Guai Code came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Guai Code with zero setup:
...
You can configure Guai Code to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for Guai Code to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your Guai Code config.
...
Guai Code supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Guai Code uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Guai Code. Sign in and approve.

   :::note
   Guai Code requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Guai Code surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Guai Code forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Guai Code is experimental. Features, configuration, and
behavior may change in future releases.
:::

Guai Code integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in Guai Code auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Guai Code uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Guai Code
...
When a `duo-workflow-*` model is selected, Guai Code will:
...
4. Route tool execution requests through Guai Code's permission-gated tool system
...
To use your GitHub Copilot subscription with Guai Code:
...
To use GMI Cloud with Guai Code:
...
To use Google Vertex AI with Guai Code:
...
2. Set the required environment variables:
   ...
   Set them while running Guai Code.
...
In the event you see a feature or model from Helicone that isn't configured automatically through Guai Code, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Guai Code conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Guai Code conversation listed as a separate session.
...
You can configure Guai Code to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure Guai Code to use local models through LM Studio.
...
You can configure Guai Code to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Guai Code. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Guai Code:
...
3. Copy the API key for use in Guai Code.
...
6. **Important**: Before using cloud models in Guai Code, you must pull the model information locally:
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your Guai Code config.
...
5. You can also customize them through your Guai Code config. Here's an example of specifying a provider
...
Guai Code's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Guai Code captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting Guai Code:
...
You can also customize models through your Guai Code config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Guai Code opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Guai Code refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Guai Code is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Guai Code — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Guai Code prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Guai Code polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with Guai Code. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
The `limit` fields allow Guai Code to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
2. For custom providers, check the Guai Code config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your Guai Code config.
