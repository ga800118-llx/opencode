...
Existem dois tipos de agentes no Guai Code; agentes primários e subagentes.
...
Guai Code vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Guai Code vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Guai Code vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Guai Code, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Se nenhuma temperatura for especificada, o Guai Code usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do Guai Code quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do Guai Code usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenCode Zen](/docs/zen), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
