---
title: CLI
description: Opções e comandos da CLI do Guai Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do Guai Code, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
opencode
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o Guai Code programaticamente.
...
Inicie a interface de usuário do terminal do Guai Code.
...
A CLI do Guai Code também possui os seguintes comandos.

---

### agent

Gerencie agentes para o Guai Code.
...
Anexe um terminal a um servidor backend do Guai Code já em execução, iniciado via comandos `serve` ou `web`.

```bash
opencode attach [url]
```

Isso permite usar o TUI com um backend Guai Code remoto. Por exemplo:
...
O Guai Code é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `opencode auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando o Guai Code é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no Guai Code.
...
Execute o Guai Code em modo não interativo passando um prompt diretamente.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor Guai Code em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `opencode`) |
...
Inicie um servidor Guai Code sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
opencode serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do Guai Code sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Liste todas as sessões do Guai Code.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do Guai Code.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do Guai Code.

```bash
opencode import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do Guai Code.
...
Inicie um servidor Guai Code sem cabeça com uma interface web.

```bash
opencode web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o Guai Code através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Desinstale o Guai Code e remova todos os arquivos relacionados.
...
Atualiza o Guai Code para a versão mais recente ou uma versão específica.
...
A CLI do Guai Code aceita as seguintes flags globais.
...
O Guai Code pode ser configurado usando variáveis de ambiente.
