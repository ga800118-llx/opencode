---
title: Configuração
description: Usando a configuração JSON do Guai Code.
---

Você pode configurar o Guai Code usando um arquivo de configuração JSON.

---

## Formato

O Guai Code suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
Coloque sua configuração global do Guai Code em `~/.config/opencode/opencode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Quando o Guai Code é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor Guai Code.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do Guai Code através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o Guai Code tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o Guai Code recuará para `"build"` com um aviso.
...
O Guai Code fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o Guai Code **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o Guai Code com ferramentas, hooks e integrações personalizadas.
...
Isso é útil quando você deseja restringir o Guai Code para usar apenas provedores específicos, em vez de desabilitá-los um a um.
