---
title: GitLab
description: Use o Guai Code em problemas e solicitações de mesclagem do GitLab.
---

O Guai Code se integra ao seu fluxo de trabalho do GitLab através do seu pipeline CI/CD do GitLab ou com o GitLab Duo.

Em ambos os casos, o Guai Code será executado em seus runners do GitLab.

---

## GitLab CI

O Guai Code funciona em um pipeline regular do GitLab. Você pode integrá-lo a um pipeline como um [componente CI](https://docs.gitlab.com/ee/ci/components/)

Aqui estamos usando um componente CI/CD criado pela comunidade para Guai Code — [nagyv/gitlab-opencode](https://gitlab.com/nagyv/gitlab-opencode).

---

### Recursos

- **Use configuração personalizada por trabalho**: Configure o Guai Code com um diretório de configuração personalizado, por exemplo `./config/#custom-directory` para habilitar ou desabilitar funcionalidades por invocação do Guai Code.
- **Configuração mínima**: O componente CI configura o Guai Code em segundo plano, você só precisa criar a configuração do Guai Code e o prompt inicial.
...
1. Armazene seu JSON de autenticação do Guai Code como variáveis de ambiente do tipo File em **Configurações** > **CI/CD** > **Variáveis**. Certifique-se de marcá-las como "Mascaradas e ocultas".
...
O Guai Code se integra ao seu fluxo de trabalho do GitLab.
Mencione `@opencode` em um comentário, e o Guai Code executará tarefas dentro do seu pipeline CI do GitLab.
...
- **Corrigir e implementar**: Peça ao Guai Code para corrigir um problema ou implementar uma funcionalidade.
  Ele criará um novo branch e abrirá uma solicitação de mesclagem com as alterações.
- **Seguro**: O Guai Code é executado em seus runners do GitLab.

---

### Configuração

O Guai Code é executado em seu pipeline CI/CD do GitLab, aqui está o que você precisará para configurá-lo:
...
Aqui estão alguns exemplos de como você pode usar o Guai Code no GitLab.
...
- **Explicar um problema**
  ...
  O Guai Code lerá o problema e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  O Guai Code criará um novo branch, implementará as alterações e abrirá uma solicitação de mesclagem com as alterações.
...
- **Revisar solicitações de mesclagem**
  ...
  O Guai Code revisará a solicitação de mesclagem e fornecerá feedback.
