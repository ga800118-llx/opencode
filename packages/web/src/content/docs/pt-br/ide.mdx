---
title: IDE
description: A extensão Guai Code para VS Code, Cursor e outros IDEs
---

O Guai Code integra-se com VS Code, Cursor ou qualquer IDE que suporte um terminal. Basta executar `opencode` no terminal para começar.

---

## Uso

- **Lançamento Rápido**: Use `Cmd+Esc` (Mac) ou `Ctrl+Esc` (Windows/Linux) para abrir o Guai Code em uma visualização de terminal dividido, ou focar em uma sessão de terminal existente se uma já estiver em execução.
- **Nova Sessão**: Use `Cmd+Shift+Esc` (Mac) ou `Ctrl+Shift+Esc` (Windows/Linux) para iniciar uma nova sessão de terminal Guai Code, mesmo que uma já esteja aberta. Você também pode clicar no botão Guai Code na interface.
- **Consciência de Contexto**: Compartilhe automaticamente sua seleção ou aba atual com o Guai Code.
...
Para instalar o Guai Code no VS Code e forks populares como Cursor, Windsurf, VSCodium:
...
Procure por **Guai Code** no Marketplace de Extensões e clique em **Instalar**.
