...
Você pode adicionar ferramentas externas ao Guai Code usando o _Model Context Protocol_, ou MCP. O Guai Code suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do Guai Code](https://opencode.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
O Guai Code lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o Guai Code irá:
...
Se o servidor requer autenticação, o Guai Code solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `opencode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o Guai Code armazenará os tokens de forma segura em `~/.local/share/opencode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no Guai Code, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do Guai Code como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o Guai Code à sua conta do Sentry.
