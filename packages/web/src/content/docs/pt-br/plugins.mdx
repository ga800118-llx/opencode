---
title: Plugins
description: Escreva seus próprios plugins para estender o Guai Code.
---

Plugins permitem que você estenda o Guai Code conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do Guai Code.
...
O Guai Code executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do Guai Code para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o Guai Code.
...
:::note
Se você estiver usando o aplicativo desktop Guai Code, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o Guai Code de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao Guai Code:
...
O helper `tool` cria uma ferramenta personalizada que o Guai Code pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o Guai Code junto com as ferramentas integradas.
