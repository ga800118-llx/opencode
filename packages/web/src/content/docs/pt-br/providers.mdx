---
title: Provedores
description: Usando qualquer provedor LLM no Guai Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Guai Code usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do Guai Code.
...
Você pode personalizar os provedores através da seção `provider` na sua configuração do Guai Code.
...
OpenCode Zen é uma lista de modelos fornecidos pela equipe do OpenCode que foram testados e verificados para funcionar bem com o Guai Code. [Saiba mais](/docs/zen).
...
Funciona como qualquer outro provedor no Guai Code e é completamente opcional.
...
testados e verificados para funcionar bem com o Guai Code.
...
Para usar o Amazon Bedrock com o Guai Code:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o Guai Code:
...
:::info
Usar sua assinatura Claude Pro/Max no Guai Code não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o Guai Code para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o Guai Code funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do Guai Code.
...
armazenar o token no armazenamento de autenticação do Guai Code.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
Guai Code usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o Guai Code
...
Para usar sua assinatura do GitHub Copilot com o Guai Code:
...
Para usar o Google Vertex AI com o Guai Code:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o Guai Code.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do Guai Code, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do Guai Code como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do Guai Code listada como uma sessão separada.
...
Você pode configurar o Guai Code para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o Guai Code para usar modelos locais através do LM Studio.
...
Você pode configurar o Guai Code para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o Guai Code. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/opencode) para detalhes.
:::
...
Para usar o Ollama Cloud com o Guai Code:
...
3. Copie a chave da API para uso no Guai Code.
...
6. **Importante**: Antes de usar modelos em nuvem no Guai Code, você deve puxar as informações do modelo localmente:
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do Guai Code.
...
5. Você também pode personalizá-los através da sua configuração do Guai Code. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o Guai Code:
...
Você também pode personalizar modelos através da sua configuração do Guai Code. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o Guai Code. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
Os campos `limit` permitem que o Guai Code entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
2. Para provedores personalizados, verifique a configuração do Guai Code e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do Guai Code.
