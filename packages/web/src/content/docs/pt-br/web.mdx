---
title: Web
description: Usando o Guai Code no seu navegador.
---

O Guai Code pode ser executado como uma aplicação web no seu navegador, proporcionando a mesma poderosa experiência de codificação com IA sem precisar de um terminal.

![Guai Code Web - Nova Sessão](../../../assets/web/web-homepage-new-session.png)
...
Isso inicia um servidor local em `127.0.0.1` com uma porta aleatória disponível e abre automaticamente o Guai Code no seu navegador padrão.
...
Por padrão, o Guai Code escolhe uma porta disponível. Você pode especificar uma porta:
...
Por padrão, o servidor se vincula a `127.0.0.1` (apenas localhost). Para tornar o Guai Code acessível na sua rede:

```bash
opencode web --hostname 0.0.0.0
```

Ao usar `0.0.0.0`, o Guai Code exibirá endereços locais e de rede:
...
Uma vez iniciada, a interface web fornece acesso às suas sessões do Guai Code.

### Sessões

Visualize e gerencie suas sessões a partir da página inicial. Você pode ver sessões ativas e iniciar novas.

![Guai Code Web - Sessão Ativa](../../../assets/web/web-homepage-active-session.png)
