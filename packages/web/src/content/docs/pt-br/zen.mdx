...
O Zen funciona como qualquer outro provedor no Guai Code. Você faz login no OpenCode Zen e recebe sua chave de API. É **completamente opcional** e você não precisa usá-lo para usar o Guai Code.
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o Guai Code.
:::
...
O OpenCode Zen funciona como qualquer outro provedor no Guai Code.
...
O [model id](/docs/config/#models) na sua configuração do Guai Code usa o formato `opencode/<model-id>`. Por exemplo, para GPT 5.5, você usaria `opencode/gpt-5.5` na sua configuração.
...
Você pode notar [modelos de baixo custo](/docs/config/#models), como Haiku, Nano ou Flash, no seu histórico de uso. O Guai Code usa esses modelos para gerar títulos de sessões.
...
4. Não ter **lock-in**, permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o Guai Code também.
