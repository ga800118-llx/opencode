---
title: Поддержка ACP
description: Используйте Guai Code в любом ACP-совместимом редакторе.
---

Guai Code поддерживает [Agent Client Protocol](https://agentclientprotocol.com) (ACP), что позволяет использовать его непосредственно в совместимых редакторах и IDE.
...
Чтобы использовать Guai Code через ACP, настройте свой редактор для запуска команды `opencode acp`.

Команда запускает Guai Code как ACP-совместимый подпроцесс, который взаимодействует с вашим редактором через JSON-RPC через stdio.
...
Чтобы открыть его, используйте новый агент Guai Code в селекторе агентов AI Chat.
...
Чтобы использовать Guai Code в качестве агента ACP в [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), добавьте в конфигурацию Neovim следующее:
...
Эта конфигурация настраивает CodeCompanion для использования Guai Code в качестве агента ACP для чата.
...
Guai Code через ACP работает так же, как и в терминале. Поддерживаются все функции:
...
- Серверы MCP, настроенные в вашей конфигурации Guai Code
