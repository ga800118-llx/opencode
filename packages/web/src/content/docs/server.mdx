---
title: Server
description: Interact with Guai Code server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `opencode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an Guai Code client can use.
...
:::tip
Use the Guai Code server to interact with Guai Code programmatically.
:::

This architecture lets Guai Code support multiple clients and allows you to interact with Guai Code programmatically.

You can run `opencode serve` to start a standalone server. If you have the
Guai Code TUI running, `opencode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Guai Code [IDE](/docs/ide) plugins.
...
The Guai Code server exposes the following APIs.
