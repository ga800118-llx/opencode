...
With Guai Code you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Guai Code uses our own `opencode` theme.
...
Guai Code comes with several built-in themes.
...
- Want Guai Code to match their terminal's appearance
...
Guai Code supports a flexible JSON-based theme system that allows users to create and customize themes easily.
