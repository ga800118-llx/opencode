...
Tools allow the LLM to perform actions in your codebase. Guai Code comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Guai Code.
...
To enable when launching Guai Code:
