---
title: CLI
description: Guai Code CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Guai Code CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
opencode
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, Guai Code ile programlı olarak etkileşim kurmanıza olanak tanır.
...
Guai Code terminal kullanıcı arayüzünü başlatır.
...
Guai Code CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

Guai Code için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir Guai Code arka uç sunucusuna bir terminal ekleyin.

```bash
opencode attach [url]
```

Bu, TUI öğesinin uzak bir Guai Code arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
Guai Code, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `opencode auth login` kullanabilirsiniz. Bu `~/.local/share/opencode/auth.json`'de saklanıyor.

```bash
opencode auth login
```

Guai Code başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları Guai Code'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek Guai Code'u etkileşimli olmayan modda çalıştırın.
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir Guai Code sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `opencode`) |
...
API erişimi için headless bir Guai Code sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
opencode serve
```

Bu, TUI arayüzü olmadan Guai Code işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `opencode` şeklindedir).
...
Guai Code oturumlarını yönetin.
...
Tüm Guai Code oturumlarını listeleyin.
...
Guai Code oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya Guai Code paylaşımından URL oturum verilerini içe aktarın.

```bash
opencode import <file>
```

Yerel bir dosyadan veya Guai Code paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir Guai Code sunucusu başlatın.

```bash
opencode web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla Guai Code'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `opencode` şeklindedir).
...
Guai Code'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
Guai Code'u en son sürüme veya belirli bir sürüme günceller.
...
Guai Code CLI aşağıdaki global bayrakları destekler.
...
Guai Code ortam değişkenleri kullanılarak yapılandırılabilir.
