...
_Model Bağlam Protokolü_ veya MCP kullanarak Guai Code'a harici araçlar ekleyebilirsiniz. Guai Code hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [Guai Code Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Guai Code, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde Guai Code şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa Guai Code, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `opencode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından Guai Code, belirteçleri `~/.local/share/opencode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra Guai Code'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi Guai Code yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve Guai Code'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
