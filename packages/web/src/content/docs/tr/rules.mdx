---
title: Kurallar
description: Guai Code için özel talimatları ayarlayın.
---

Bir `AGENTS.md` dosyası oluşturarak Guai Code için özel talimatlar verebilirsiniz. Bu, Cursor rules yaklaşımına benzer. Özel projeniz için davranışını özelleştirmek amacıyla LLM'in içeriğine dahil edilecek talimatları içerir.

---

## Başlat

Yeni bir `AGENTS.md` dosyası oluşturmak için `/init` komutunu Guai Code'da çalıştırabilirsiniz.

:::tip
Projenizin `AGENTS.md` dosyasını Git'e göndermelisiniz.
:::

Bu, projenin neyle ilgili olduğunu anlamak için projenizi ve tüm içeriğini tarayacak ve onunla bir `AGENTS.md` dosyası oluşturacaktır. Bu, Guai Code'un projede daha iyi gezinmesine yardımcı olur.
...
Guai Code ayrıca `AGENTS.md` dosyasının birden fazla konumdan okunmasını da destekler. Ve bu farklı amaçlara hizmet ediyor.
...
Ayrıca `~/.config/opencode/AGENTS.md` dosyasında genel kurallara sahip olabilirsiniz. Bu, tüm Guai Code oturumlarına uygulanır.
...
Guai Code, Claude Code'dan geçiş yapan kullanıcılar için yedek olarak Claude Code'un dosya kurallarını destekler:
...
`AGENTS.md`'nizde açık talimatlar sağlayarak Guai Code'u harici dosyaları okumayı öğretebilirsiniz. İşte pratik bir örnek:
...
- Guai Code'un dosyaları yalnızca belirli bir görev için gerektiğinde yüklediğinden emin olun
