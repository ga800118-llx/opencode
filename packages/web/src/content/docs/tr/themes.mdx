...
Guai Code ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak Guai Code kendi `opencode` temasını kullanır.
...
Guai Code birden fazla yerleşik temayla gelir.
...
- Guai Code'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
Guai Code, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
