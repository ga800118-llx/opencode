...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. Guai Code yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
Guai Code içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca Guai Code provider kullanılırken veya `OPENCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

Guai Code başlatırken etkinleştirmek için:
