---
title: TUI
description: Guai Code terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Guai Code, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Guai Code'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Guai Code TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Guai Code'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Guai Code'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
- `leader_timeout` - Guai Code'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
