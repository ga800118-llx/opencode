---
title: Web
description: Tarayıcınızda Guai Code'u kullanma.
---

Guai Code, tarayıcınızda bir web uygulaması olarak çalışabilir ve bir terminale ihtiyaç duymadan aynı güçlü AI kodlama deneyimini sağlayabilir.

![Guai Code Web - Yeni Oturum](../../../assets/web/web-homepage-new-session.png)
...
Bu, `127.0.0.1` üzerinde rastgele kullanılabilir bir bağlantı noktasına sahip yerel bir sunucuyu başlatır ve Guai Code'u varsayılan tarayıcınızda otomatik olarak açar.
...
Guai Code varsayılan olarak kullanılabilir bir bağlantı noktasını seçer. Bir bağlantı noktası belirtebilirsiniz:
...
Varsayılan olarak sunucu `127.0.0.1` (yalnızca localhost) öğesine bağlanır. Guai Code'u ağınızda erişilebilir kılmak için:

```bash
opencode web --hostname 0.0.0.0
```

`0.0.0.0` kullanıldığında, Guai Code hem yerel hem de ağ adreslerini gösterecektir:
...
Web arayüzü başlatıldığında Guai Code oturumlarınıza erişim sağlar.

### Oturum

Oturumlarınızı ana sayfadan görüntüleyin ve yönetin. Aktif oturumları görebilir ve yenilerini başlatabilirsiniz.

![Guai Code Web - Etkin Oturum](../../../assets/web/web-homepage-active-session.png)

### Sunucu Durumu

Bağlı sunucuları ve durumlarını görüntülemek için "Sunucuları Gör" seçeneğini tıklayın.

![Guai Code Web - Sunuculara bakın](../../../assets/web/web-homepage-see-servers.png)
