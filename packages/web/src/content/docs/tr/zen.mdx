---
title: Zen
description: Guai Code tarafından sağlanan seçilmiş modeller listesi.
---
...
Zen, Guai Code'daki diğer sağlayıcılar gibi çalışır. OpenCode Zen'de oturum açıp API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Guai Code kullanmak için buna ihtiyacınız yoktur.
...
:::tip
Guai Code ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
OpenCode Zen, Guai Code'daki diğer sağlayıcılar gibi çalışır.
...
Guai Code yapılandırmanızdaki [model id](/docs/config/#models) `opencode/<model-id>` biçimini kullanır. Örneğin, GPT 5.5 için yapılandırmanızda `opencode/gpt-5.5` kullanırsınız.
...
Kullanım geçmişinizde Haiku, Nano veya Flash gibi [düşük maliyetli modeller](/docs/config/#models) görebilirsiniz. Guai Code, oturum başlıklarını oluşturmak için bu modelleri kullanır.
...
4. Başka herhangi bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenme olmamasını** sağlamak. Ayrıca Guai Code ile her zaman başka herhangi bir sağlayıcıyı da kullanabilmenizi sağlamak.
