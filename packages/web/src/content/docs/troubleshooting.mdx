...
To debug issues with Guai Code, start by checking the logs and local data it stores on disk.
...
Guai Code stores session data and other application data on disk at:
...
To uninstall the Guai Code CLI and remove its related files, run:
...
To remove Guai Code Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

Guai Code Desktop runs a local Guai Code server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Guai Code` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
Guai Code can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Guai Code can rebuild it.

1. Quit Guai Code Desktop completely.
...
3. Restart Guai Code Desktop.

---

### Fix server connection issues

Guai Code Desktop can either start its own local server (default) or connect to a server URL you configured.
...
On Windows, Guai Code Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Guai Code's features.

---

### Notifications not showing

Guai Code Desktop only shows system notifications when:

- notifications are enabled for Guai Code in your OS settings, and
...
1. Quit Guai Code Desktop.
2. Find and delete these files (they live in the Guai Code Desktop app data directory):
...
If you're experiencing issues with Guai Code:
...
If you encounter API call errors, this may be due to outdated provider packages. Guai Code dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart Guai Code to reinstall the latest provider packages

This will force Guai Code to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
Guai Code will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
