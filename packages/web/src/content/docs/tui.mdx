---
title: TUI
description: Using the Guai Code terminal user interface.
---
...
Running Guai Code starts the TUI for the current directory.
...
When using the Guai Code TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Guai Code. Allows you to select from available providers and add their API keys.
...
Exit Guai Code. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long Guai Code waits after the leader key. Defaults to `2000`.
