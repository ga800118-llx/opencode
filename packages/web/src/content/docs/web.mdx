---
title: Web
description: Using Guai Code in your browser.
---

Guai Code can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![Guai Code Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens Guai Code in your default browser.
...
By default, Guai Code picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make Guai Code accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, Guai Code will display both local and network addresses:
...
Once started, the web interface provides access to your Guai Code sessions.
