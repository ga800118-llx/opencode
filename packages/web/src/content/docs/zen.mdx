...
Zen works like any other provider in Guai Code. You login to OpenCode Zen and get
...
:::tip
We tested a select group of models and providers that work well with Guai Code.
:::
...
OpenCode Zen works like any other provider in Guai Code.
...
The [model id](/docs/config/#models) in your Guai Code config
...
You may notice [low-cost models](/docs/config/#models), such as Haiku, Nano, or Flash, in your usage history. Guai Code uses these models to generate session titles.
...
4. Have **no lock-in** by allowing you to use it with any other coding agent. And always let you use any other provider with Guai Code as well.
