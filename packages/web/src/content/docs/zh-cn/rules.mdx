---
title: 规则
description: 为 Guai Code 设置自定义指令。
---

您可以通过创建 `AGENTS.md` 文件来为 Guai Code 提供自定义指令。这类似于 Cursor 的规则功能。该文件包含的指令会被纳入 LLM 的上下文中，以便针对您的特定项目自定义其行为。

---

## 初始化

要创建新的 `AGENTS.md` 文件，您可以在 Guai Code 中运行 `/init` 命令。

:::tip
您应该将项目的 `AGENTS.md` 文件提交到 Git。
:::

该命令会扫描您的项目及其所有内容，了解项目的用途，并据此生成一个 `AGENTS.md` 文件。这有助于 Guai Code 更好地导航您的项目。
...
Guai Code 还支持从多个位置读取 `AGENTS.md` 文件，不同的位置有不同的用途。
...
您还可以在 `~/.config/opencode/AGENTS.md` 文件中设置全局规则。这些规则会应用于所有 Guai Code 会话。

由于该文件不会被提交到 Git 或与团队共享，我们建议用它来指定 LLM 应遵循的个人规则。

### Claude Code 兼容性

对于从 Claude Code 迁移过来的用户，Guai Code 支持 Claude Code 的文件约定作为回退方案：
...
虽然 Guai Code 不会自动解析 `AGENTS.md` 中的文件引用，但您可以通过以下两种方式实现类似的功能：
...
您可以在 `AGENTS.md` 中提供明确的指令，教 Guai Code 读取外部文件。以下是一个实际示例：
