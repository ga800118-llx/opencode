...
Guai Code 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev) 支援 **75+ LLM 提供商**，並支援執行本地模型。

---

## 提供商

大多數熱門提供商已預設預先載入。如果您透過 `/connect` 指令新增了提供商的憑證，它們將在您啟動 Guai Code 時自動可用。
...
以下是與 Guai Code 配合良好的幾個模型，排名不分先後（此列表並非詳盡無遺，也不一定是最新的）：
...
要將某個模型設為預設模型，可以在 Guai Code 設定中設定 `model` 欄位。
...
許多模型支援具有不同設定的多種變體。Guai Code 為熱門提供商內建了預設變體。

### 內建變體

Guai Code 為許多提供商提供了預設變體：
...
Guai Code 啟動時，會按以下優先順序載入模型：

1. `--model` 或 `-m` 命令列旗標。格式與設定檔中相同：`provider_id/model_id`。

2. Guai Code 設定中的 model 欄位。
