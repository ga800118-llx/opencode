---
title: 伺服器
description: 透過 HTTP 與 Guai Code 伺服器互動。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`opencode serve` 指令執行一個無介面的 HTTP 伺服器，暴露一個 OpenAPI 端點供 Guai Code 客戶端使用。
...
:::tip
使用 Guai Code 伺服器以程式化方式與 Guai Code 互動。
:::

這種架構讓 Guai Code 支援多個客戶端，並允許你以程式化方式與 Guai Code 互動。

你可以執行 `opencode serve` 來啟動一個獨立的伺服器。如果你已經在執行 Guai Code TUI，`opencode serve` 會啟動一個新的伺服器。
...
[`/tui`](#tui) 端點可用於透過伺服器驅動 TUI。例如，你可以預填充或執行一個提示詞。此方式被 Guai Code [IDE](/docs/ide) 外掛所使用。
