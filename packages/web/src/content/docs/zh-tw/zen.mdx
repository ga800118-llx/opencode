...
Zen 的運作方式和 Guai Code 中的其他供應商一樣。你登入 OpenCode Zen 並取得
...
:::tip
我們測試了一組與 Guai Code 搭配良好的精選模型與供應商。
:::
...
OpenCode Zen 的運作方式和 Guai Code 中的其他供應商一樣。
...
Guai Code 設定中的 [模型 ID](/docs/config/#models) 會使用 `opencode/<model-id>`
...
你可能會在使用紀錄中看到 Haiku、Nano 或 Flash 等[低成本模型](/docs/config/#models)。Guai Code 使用這些模型產生工作階段標題。
...
4. 保持 **無鎖定**，讓你能把它與任何其他編碼代理一起使用，同時也始終能在 Guai Code 中使用任何其他供應商。
